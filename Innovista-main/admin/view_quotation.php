...
$stmt = $conn->prepare("
    SELECT q.id AS request_id, q.service_type, q.project_description, q.status AS request_status, q.created_at AS request_created_at,
           c.name AS customer_name, c.email AS customer_email,
           p.name AS provider_name, p.email AS provider_email,
           cq.id AS custom_quote_id, cq.quotation_id, cq.amount AS quoted_amount, cq.advance, cq.start_date, cq.end_date,
...
$project_data = null;
if ($quotation_data['custom_quote_id']) { // Assuming project is linked to custom_quotations
...
if ($quotation_data['custom_quote_id']) {
    $stmt_payments = $conn->prepare("SELECT amount, payment_type, transaction_id, payment_date FROM payments WHERE quotation_id = :custom_quote_id ORDER BY payment_date DESC");
...
<div class="content-card">
    ...
    <div class="form-grid">
        <div class="detail-item"><strong>Customer:</strong> <?php echo htmlspecialchars($quotation_data['customer_name']); ?> (<?php echo htmlspecialchars($quotation_data['customer_email']); ?>)</div>
        <div class="detail-item"><strong>Provider:</strong> <?php echo htmlspecialchars($quotation_data['provider_name']); ?> (<?php echo htmlspecialchars($quotation_data['provider_email']); ?>)</div>
    ...
    <div class="detail-item mt-3">
        ...
        <p><?php echo nl2br(htmlspecialchars($quotation_data['project_description'])); ?></p>
    </div>
    <?php if ($quotation_data['custom_quote_photos']): ?>
        <div class="detail-item mt-3">

...
$pending_quotes_query = $db->prepare("SELECT prov.name as provider_name, q.project_description, q.id as quotation_id FROM quotations q JOIN users prov ON q.provider_id = prov.id WHERE q.customer_id = :id AND q.status = 'quote_sent' ORDER BY q.id DESC");
...
$completed_projects_query = $db->prepare("
    ...
    WHERE q.customer_id = :id AND p.status = 'completed'
    ORDER BY p.id DESC
");
...
<div class="dashboard-section">
    ...
    <div class="content-card">
        <div class="table-wrapper">
            <table>
                <thead>
                  <tr><th>Provider</th><th>Project</th><th>Status</th><th>Action</th></tr>
                </thead>
                <tbody>
                    <?php if (!empty($completed_projects)): foreach ($completed_projects as $project): ?>
                    <tr>
                        <td><?= htmlspecialchars($project['provider_name']); ?></td>
                        <td><?= htmlspecialchars($project['project_description']); ?></td>
                        <td><span class="status-badge status-approved">Completed</span></td>
                        <td>
                            <a href="view_project_history.php?id=<?= $project['quotation_id']; ?>" class="btn-view">View Project</a>

                            <?php
                              require_once __DIR__ . '/../classes/Review.php';
                              $reviewObj = new Review($db);

                              // Check if feedback exists using quotation_id (because reviews are stored by quotation_id)
                              $feedback = $reviewObj->getFeedbackByBooking($project['quotation_id'], $customer_id);
                            ?>

                            <?php if (!$feedback): ?>
                                <a href="feedback_form.php?booking_id=<?= $project['booking_id']; ?>" class="btn-view">Give Feedback</a>
                            <?php else: ?>
                                <a href="view_feedback.php?booking_id=<?= $project['booking_id']; ?>" class="btn-view">View Feedback</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; else: ?>
